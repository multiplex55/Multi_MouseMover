...
| `1` / `2` / `3` / `4` / `5` / `6` / `7` / `8` / `9` | Select bookmark slot 1 ... 9 (in bookmark mode) |
| `H` | Navigate back |
| `Y` | Navigate forward |
| `Q` / `P` | Switch to idle |
...
`[status_overlay]` controls the compact persistent status indicator. It can show active/idle, drag, slow movement, surgical held precision state, jump state, speed flashes, wheel state, and final-adjust state.
